...
export const Footer = props => {
  ...
  return (
    <>
      ...
      <footer className='wow fadeInUp relative mt-6 z-10 bg-[#090E34] pt-20 lg:pt-[100px]' data-wow-delay='.15s'>
            ...
            {CONFIG.STARTER_FOOTER_LINK_GROUP?.map((item, index) => {
              return (
                <div key={index} className='w-full px-4 sm:w-1/2 md:w-1/2 lg:w-2/12 xl:w-2/12'>
                  <div className='mb-10 w-full'>
                    <h4 className='mb-9 text-lg font-semibold text-white'>{item.TITLE}</h4>
                    <ul>
                      {item?.LINK_GROUP?.map((l, i) => {
                        return (
                          <li key={i}>
                            <a href={l.URL} className='mb-3 inline-block text-base text-gray-7 hover:text-primary'>
                              {l.TITLE}
                            </a>
                          </li>
                        )
              ...
              <div className='mb-10 w-full'>
                ...
                <div className='flex flex-col gap-8'>
                  {latestPosts?.map((item, index) => {
                    ...
                    return (
                      <a key={index} href={url} className='group flex items-center gap-[22px]'>
