    ...
    <div
      ...
      <div
        ...
        <div className='flex justify-between items-center'>
          <div className='text-2xl text-blue-600 dark:text-yellow-600 font-bold'>Search</div>
          <div>
            <i
        ...
        <ul className='flex-1 overflow-auto'>
          {searchResults.map((result, index) => (
            <li
              ...
              <a className={`${activeIndex === index ? ' text-white' : ' text-black dark:text-gray-300 '}`}>
                {result.title}
              </a>
            </li>
          ))}
        ...
        <div className='flex items-center justify-between mt-2 sm:text-sm text-xs dark:text-gray-300'>
              ...
              {ShortCutActions.map((action, index) => {
                return (
                  <Fragment key={index}>
                    ...
                    <span className='ml-2 mr-4  text-gray-600 dark:text-gray-300'>{action.action}</span>
                  </Fragment>
                )
              })}
            </div>
          ...
          <div>
            {totalHit > 0 && (
              <p>
                Total search results {totalHit} Results found, Time taken {useTime} ms
              </p>
            )}
          </div>
          <div className='text-gray-600 dark:text-gray-300  text-right'>
            <span>
              <i className='fa-brands fa-algolia'></i> Provided by Algolia
            </span>
          </div>
        </div>
      </div>
  ...
  return (
    <div className='flex space-x-1 w-full justify-center py-1'>
      {Array.from({ length: totalPage }, (_, i) => {
        ...
        return (
          <div onClick={() => switchPage(i)} className={`text-center cursor-pointer w-6 h-6 ${classNames}`} key={i}>
            {i + 1}
          </div>
        )
      })}
